...
int count_digit(int n) {
	return (n == 0) ? 0 : static_cast<int>(floor(log10(n) + 1));
}
...
public:
	...
	void draw_node(int item, int centre_x, int centre_y, bool write_value = true) {
		char data_string[10];
		const int num_segments = 50; // Number of segments in the circle approximation
		const float radius = node_width;

		glBegin(GL_POLYGON);
		for (int i = 0; i < num_segments; ++i) {
			float theta = 2.0 * M_PI * float(i) / float(num_segments);
			float x = radius * cos(theta);
			float y = radius * sin(theta);
			glVertex2f(static_cast<GLfloat>(centre_x + x), static_cast<GLfloat>(centre_y + y));
		}
		glEnd();
		...
		if (write_value) {
			...
			display_string(data_string, static_cast<float>(centre_x - node_width + 10), static_cast<float>(centre_y - 5));
		}
	}
	...
	void draw_arrow(int par_x, int par_y, int node_x, int node_y, bool color = true) {
		if (color)
			...
			glColor3f(1.0, 1.0, 1.0);
		glBegin(GL_LINES);
		glVertex2f(static_cast<GLfloat>(par_x), static_cast<GLfloat>(par_y - node_width));
		glVertex2f(static_cast<GLfloat>(node_x), static_cast<GLfloat>(node_y + node_width));
		glEnd();
	}
...
public:
	...
	void draw_box() {
		...
		offset += 120.0;
		glColor3f(1.0, 1.0, 0.0);
		display_string((option_name), static_cast<float>(bottom_corner_x + 10.0), static_cast<float>(bottom_corner_y + 20.0));
	}
...
void display() {
	...
	if (is_not_found && !is_query_string) {
		glColor3f(1.0, 0.0, 0.0);
		if (!is_query_string)
			display_string(not_found, 100, 570);  // Adjust the y-coordinate based on the query string
		else
			display_string(not_found, 100, 600);  // Default y-coordinate
	}
}
